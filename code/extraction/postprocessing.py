...
def remove_indicators(relations, print_res=False):
    ...
    for rel in relations:
        ...
        if not cause or not effect:
            continue
        if (bool(INDICATOR_RE.fullmatch(cause.strip())) or bool(ALLCAPS_CODE_RE.fullmatch(cause.strip()))) and not p_val:
            removed.append(cause)
            continue
        if (bool(INDICATOR_RE.fullmatch(effect.strip())) or bool(ALLCAPS_CODE_RE.fullmatch(effect.strip()))) and not p_val:
            removed.append(effect)
            continue
        if (bool(INDICATOR_RE.fullmatch(mod.strip())) or bool(ALLCAPS_CODE_RE.fullmatch(mod.strip()))) and not p_val:
            removed.append(mod)
            continue
        cleaned.append(rel)
...
def clean_predictions(base_dir_pred, 
    ...
    for fname in tqdm(sorted(files)):
        ...
        before = len(model_output['relations'])
        ...
        rels = check_valid(rels)
        rels = format_pvalues(rels)
        rels = clean_names(rels, abbreviations)
        check_betas(rels, path2file)
        model_output['relations'] = rels

        after = len(model_output['relations'])
        #if before != after:
        #    print(f"{base_name}: {before} -> {after}")

        if save_results:
            with open(os.path.join(base_dir_pred, base_name + '_cleaned.json'), 'w') as outfile:
                json.dump(model_output, outfile, indent=2)
'''

def clean_relations(rels, path2file, vision=False):
    abbreviations = get_paper_abbreviations(path2file)
    rels, _ = remove_indicators(rels)
    if vision:
        rels = transform_vision_relations(rels)
